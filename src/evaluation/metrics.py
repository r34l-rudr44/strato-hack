...
def generate_report(
    ...
    1. results = {model_name: metrics_dict, ...} - Multi-model comparison
    2. results = {'metrics': {...}, 'confusion_matrix': ...} - Single detector result
    
    Args:
        results: Results dict (multi-model or single detector)
        detector_name: Name of the detector (for single detector mode)
    ...
    """
    # Detect input format
    if 'metrics' in results and 'confusion_matrix' in results:
        # Single detector format from evaluate_detector()
...
DATA STATISTICS
---------------
Total Samples:       {metrics.get('n_samples', 'N/A')}
True Anomalies:      {metrics.get('n_anomalies_true', 'N/A')} ({metrics.get('anomaly_ratio_true', 0):.2%})
Predicted Anomalies: {metrics.get('n_anomalies_pred', 'N/A')} ({metrics.get('anomaly_ratio_pred', 0):.2%})
...
"""
        ...
        # Multi-model format: {model_name: metrics_dict, ...}
        from datetime import datetime
        
        # Find best model by F1
        best_name = max(results.items(), key=lambda x: x[1].get('f1_score', 0))[0]
        ...
        lines = [
            ...
            "-" * 40,
            f"Model:     {best_name}",
            f"Accuracy:  {best_m.get('accuracy', 0):.4f}",
            f"Precision: {best_m.get('precision', 0):.4f}",
            f"Recall:    {best_m.get('recall', 0):.4f}",
            f"F1-Score:  {best_m.get('f1_score', 0):.4f}",
        ...
        # Sort by F1
        for name, m in sorted(results.items(), key=lambda x: -x[1].get('f1_score', 0)):
            lines.append(
                f"{name:15s}: Acc={m.get('accuracy', 0):.3f}, Prec={m.get('precision', 0):.3f}, "
                f"Rec={m.get('recall', 0):.3f}, F1={m.get('f1_score', 0):.3f}, AUC={m.get('roc_auc', 0):.3f}"
            )
